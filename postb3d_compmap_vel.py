...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 13 14:56:56 2024
...
def make_vel_compmap(datapath,figpath=None,set_title=None,set_ticks=False,
                     panel_pos=None):
    ...
    if panel_pos=='bottom':
        ...
        fig,ax = setup_comparison_maps(comp_shape=(1,3), map_shape=(41,41), figsize=(15,4),xlabel='')
    plot_map(ax[0][0],precon_vel,clim=vel_lim,cmap=cmap.v)
    ...
    if set_ticks==True:
        ...
        ## todo
        set_xticks(ax[0][0],xticks='lower',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[0][1],xticks='all',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[0][2],xticks='upper',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[0][4],xticks='all',x_dist=x_dist,naxis=naxis)
        
        #set_yticks(ax[0][0],yticks='upper',y_dist=y_dist,naxis=naxis)
        set_yticks(ax[0][0],yticks='all',y_dist=y_dist,naxis=naxis)
    ...
    if set_title == None:
        ...
        plt.savefig(figpath+"compmap_"+set_title+"_vel.pdf",dpi=300,bbox_inches='tight')


def make_vel_compmap_combine(datapath_list,magpiid_list,figpath=None,set_title=None,set_ticks=False):
    ...
    for datapath in datapath_list:
        ...
        # precon flux, velocity map, velocity dispersion map
        precon_flux_ha = post_b3d.maps[sample, 0]*flux_scale_factor
        shape_list.append(precon_flux_ha.shape)
        
    max_shape = np.max(shape_list, axis=0)
    
    fig,ax = setup_comparison_maps(comp_shape=(4,3), map_shape=max_shape, figsize=(15,16))
    
    
    i=0
    for datapath,magpiid in zip(datapath_list,magpiid_list):
        ...
        precon_vel = fit_array_size(precon_vel, max_shape)
        con_vel = fit_array_size(con_vel, max_shape)
        data_vel = fit_array_size(data_vel, max_shape)
        res_vel = fit_array_size(res_vel, max_shape)
        
        
        
        plot_map(ax[i][0],precon_vel,clim=vel_lim,cmap=cmap.v,ylabel=magpiid)
        plot_map(ax[i][1],con_vel,clim=vel_lim,cmap=cmap.v)
        plot_map(ax[i][2],data_vel,clim=vel_lim,cmap=cmap.v,cbar_label='v(km/s)')
        plot_map(ax[i][4], res_vel,clim=res_vel_lim,cbar_label='$\Delta$v(km/s)',cmap=cmap.residuals)
        colorbar(mpb1,cax=ax[i][3],clim=vel_lim,label='$v$(km/s)')
        colorbar(res_mpb1,cax=ax[i][5],clim=res_vel_lim,label='$\Delta v$(km/s)')
    ...
    if set_ticks==True:
        ...
        y_dist = naxis[0]*0.2/2
        ## todo
        set_xticks(ax[3][0],xticks='lower',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[3][1],xticks='all',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[3][2],xticks='upper',x_dist=x_dist,naxis=naxis)
        set_xticks(ax[3][4],xticks='all',x_dist=x_dist,naxis=naxis)
        
        set_yticks(ax[0][0],yticks='upper',y_dist=y_dist,naxis=naxis)
        set_yticks(ax[1][0],yticks='upper',y_dist=y_dist,naxis=naxis)
        set_yticks(ax[2][0],yticks='all',y_dist=y_dist,naxis=naxis)
        set_yticks(ax[3][0],yticks='lower',y_dist=y_dist,naxis=naxis)
...
def fit_array_size(original_array,larger_shape):
    # Create a larger 2D array filled with NaN
    larger_array = np.full(larger_shape, np.nan)
    
    # Calculate the starting indices to place the original array in the middle
    start_row = (larger_shape[0] - original_array.shape[0]) // 2
    start_col = (larger_shape[1] - original_array.shape[1]) // 2
    
    # Insert the original array into the middle of the larger array
    larger_array[start_row:start_row + original_array.shape[0], start_col:start_col + original_array.shape[1]] = original_array
