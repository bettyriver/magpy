...
class PreBlobby3D:
    ...
    def __init__(
            self, dirpath, fitsname, emipath, eminame, redshift_path, save_path, wave_axis=0):
        """
        ...
        x_pix_range_sec = (np.arange(0,self.nx)+1-x_crpix)*x_del*3600
        self.x_pix_range = x_pix_range_sec - (x_pix_range_sec[0]+x_pix_range_sec[-1])/2

        y_pix_range_sec = (np.arange(0,self.ny)+1-y_crpix)*y_del*3600
        self.y_pix_range = y_pix_range_sec - (y_pix_range_sec[0]+y_pix_range_sec[-1])/2
    ...
    def model_options(self):
        ...
        modelfile.writable('LSFFWHM\t%.3f'%(lsf_deredshift))
